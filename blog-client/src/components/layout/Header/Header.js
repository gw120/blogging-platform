    ...
    <header className="h-full">
        <div className={`absolute m-4 ${navOpen ? 'hidden' : 'block'}`}>
            <button className="p-2" onClick={() => setNavOpen(true)}>
                <svg
                    className="w-8 h-8 fill-current text-gray-100 cursor-pointer"
        ...
        <Nav navNode={navNode} navOpen={navOpen} setNavOpen={setNavOpen} />
        <div className="h-full flex justify-center items-center px-1">
            <h1 className="text-4xl text-center transformOrigin-1">
                Blogging Platform
            </h1>
        </div>
    </header>
